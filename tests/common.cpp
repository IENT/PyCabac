...
void fillVectorRandomUniform(uint64_t min, uint64_t max, std::vector<unsigned int> *const vector) {
    ...
    std::uniform_int_distribution<unsigned int> dist(0, max - min);
...
void fillVectorRandomGeometric(std::vector<unsigned int> *const vector) {
    // First create an instance of an engine.
    std::random_device rnd_device;
    // Specify the engine and distribution.
    auto seed = rnd_device();
    std::mt19937_64 mersenne_engine{seed};  // Generates random integers
    std::cout << "fillVectorRandomGeometric: len: " << vector->size() << "; seed: " << seed << std::endl;

    std::geometric_distribution<unsigned int> dist(0.05);
